 ...
 * @returns
 */
function next(type, from){
	var form = new mini.Form("#form1");
	form.validate();
	if(!form.isValid()) return;
	var jsonTemplates = getTemplate();
	if(jsonTemplates=="{}"){
		mini.alert("请去表单模板新增或初始化模板");
		return false;
	}
	var templates=encodeURI(jsonTemplates );
	if(type=="mobile"){
		var url=__rootPath + "/bpm/form/bpmMobileForm/edit.do?boDefId=" + boDefId +"&templates=" + templates;
		_OpenWindow({
			url: url,
		...
		initAgain(templates, from);
	}
}

function initAgain(templates, from){
	debugger
	...
	var designTemplate;
	var url = __rootPath + "/bpm/form/bpmFormView/edit.do?";
	if(boDefId) {
		url += 'fromEntBoDefId='+boDefId +"&templates=" + templates +"&color="+color +"&genTab="+genTab+"&viewId="+viewId+"&boDefId="+boDefId+"&fromDesign=true";
	} else {
		url += 'fromEntBoDefId='+fromEntBoDefId +"&templates=" + templates +"&color="+color +"&genTab="+genTab+"&viewId="+viewId+"&fromDesign=true";
	}
	_OpenWindow({
		title: '新增业务表单视图',
		max: true,
