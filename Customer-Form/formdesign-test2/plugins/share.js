...
function _ShowTips(config){
    ...
    $(document).on('click' , '.mini-tips .icon-tips-Close' , function(){//提前关闭弹窗
...
function _ShowErr(config){
	...
	mini.showTips({
         ...
         content:"<em class='MyCenter colorRed'>" +
         			"<div class='hint'><h1>处理结果</h1><i class='icon-tips-Close'></i></div>"+
         			"<div  class='MyContent'>" +
         				"<div><span class='MyYuan'>!</span></div>" +
         				"<h4><span></span>"+config.content+"</h4>" +
...
Date.prototype.subtract = function(date,format,fixed){
	  ...
	  switch(format){
	    //分钟
	  	...
	  	//天数
	  	default :
	  		if(day==0){//说明俩个日期的时间间距小于1天，但日期有可能是不在同一天;
	  			rtn = this.getDate()-date.getDate();
			}else{
				rtn=day;
			}
	  }
	  if(fixed){
	  	var num = Math.pow(10,fixed);
		  rtn= Math.ceil(rtn * num)/num;
	  }else{
	  	rtn = parseInt(rtn);
	  }
	  return rtn;
 ...
 * 自定义按钮入参编译
 */
function changegridInputValue(binding) {
    var gridInputs=binding.gridInput;
    if(gridInputs && gridInputs.length>0){
        for(key in gridInputs){
        	var gridInput =gridInputs[key];
        	if("YES"==gridInput.autoFilter){
				if(gridInput.fieldName.indexOf("Q_")==-1){
					var fieldName="Q_"+gridInput.fieldName+"_"+gridInput.queryDataType+"_"+gridInput.fieldOp;
					gridInput.fieldName=fieldName;
				}
			}
		}
	}
	...
	var conf={
			...
			ondestroy:function(data){
				...
				if(isMain){//是否是主表
                    ...
                    //更新行。
                    updateRowExt(subGrid, row, rowData);
...
function Print(){
	...
	//解析编辑器的内容
	var arys=mini.findControls(function(control){
		if(control.type=="mini-editor"){
			return true;
		}
	});
	var temps=parseEditor(arys);
...
function parseEditor(arys){
	var temps={};
	for(var i=0;i<arys.length;i++){
		var obj=arys[i];
		var bodyEl=obj.getBodyEl();
		var html=bodyEl.innerHTML;
		var val=obj.getValue();
		var id=obj.uid +"_body";
		var hei=$("#" + id).height();
		...
		$("#" + id).parents(".mini-ueditor").height(height).css("padding",1);
		temps[id]={content:html,height:hei,style:style};
	}
	return temps;
}

function revertEditor(temps){
	for(var key in temps){
		$("#" + key).html(temps[key].content);
		$("#" + key).attr("style",temps[key].style);
		$("#" + key).parent().height(temps[key].height);
		$("#" + key).parents(".mini-ueditor").height(temps[key].height);
		$("#" + key).parents(".mini-ueditor").css("padding",0);
	}
}
...
function subExcelImport(e){

	var data=e.source;//获取excel配置信息
	var gridId = data.formData.name;
	_OpenWindow({
		url:__rootPath+'/sys/core/sysBoList/subExcelImport.do',
		title:'子表excel导入',
		height:400,
		width:700,
		onload:function(){
		...
		ondestroy:function(action){
			...
			if(action=="ok"){
				...
				var gridData = mini.get("grid_"+gridId);
				gridData.addRows(JSON.parse(data));//添加
			}
		}
	});
}
 ...
 * 更新表格行数据。
 *
 * 这个作用是在调用表格更新行数据时执行其他的自定义函数。
 * @param grid
 * @param row
 * @param updObj
 */
function updateRowExt(grid, row, updObj) {
    grid.updateRow(row, updObj);

    if (grid.updateRowExt) {
        grid.updateRowExt(row);
    }
}