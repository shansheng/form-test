...
function validType(e) {
	...
	var val=e.value;
	var aryType=e.vtype.split(",");
	for(var i=0;i<aryType.length;i++){
		var type=aryType[i];
		var isValid=mini.VTypes[type](val);
		if(!isValid){
			var errTxt=mini.VTypes[type +"ErrorText"];
            e.errorText=errTxt;
            e.isValid = false;
			break;
		}
...
function getPy(name,val){
	...
	$.ajax({
		...
		method:'POST',
		async:false,
		data:{words:vals,isCap:'false',isHead:'true'},
		success:function(result){
...
function checkPy(val) {
	var vals = stripscript(val);
	var flag = true;
	if (vals.length != val.length) flag = false;
	return flag;
}
