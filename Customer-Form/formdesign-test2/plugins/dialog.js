 ...
 * @returns
 */
function _openFieldToBoDialog(conf){
	_OpenWindow({
		title:'选择表单字段',
		height:450,
		width:680,
		url:__rootPath+'/bpm/core/bpmSolution/modelBosDlg.do?solId='+conf.solId+'&nodeId='+conf.nodeId+'&actDefId='+conf.actDefId,
		ondestroy:function(action){
			if(action!='ok') return;
			var iframe=this.getIFrameEl();
...
function openFieldDialog(conf){
	var url = __rootPath+'/bpm/core/bpmSolution/modelFieldsDlg.do?solId='+conf.solId+'&nodeId='+conf.nodeId+'&actDefId='+conf.actDefId;
	if(conf.boDefId){
 ...
 * @returns
 */
function previewWord(alias,id,jsonConfig){
	...
	if(aliasArray.length>1){
		_OpenWindow({
			...
			width:800,
			height:550,
			url:__rootPath +"/sys/core/sysWordTemplate/wordMultList.do",
            onload:function(){

                var iframe = this.getIFrameEl().contentWindow;
                iframe.init(jsonConfig);
            },
			ondestroy:function(action){
				if(action=='ok'){
					var iframe = this.getIFrameEl();
					var data = iframe.contentWindow.getGridData();//拿到excel的数据
					alias = data.TEMPLATE_NAME_;
					preView(alias,id);
				}
			}
		});
...
function _BtnPreviewWord(e){
	...
	var pk=btn.getPk() || "ID_";
	debugger
	var val=mini.getByName(pk, parent[0]).getValue();
	if(!val) return;
	previewWord(btn.alias,val,btn.jsonConfig);
}
...
function _SaasGroupDlg(conf){
	...
	var url =__rootPath+'/sys/org/osGroup/dialog.do?single='+conf.single+'&reDim='+conf.reDim+'&showDimId='+conf.showDimId;
	url =url+'&tenantId='+conf.tenantId +'&excludeAdmin='+conf.excludeAdmin+"&isGrade="+conf.isGrade;
	_OpenWindow({
...
function _TenantGroupDlg1(tenantId,single,config,showDimId,callback,reDim,groupIdList){
	...
	_OpenWindow({
		iconCls:'icon-group-dialog',
		url:__rootPath+'/sys/org/osGroup/dialog.do?single='+single+'&reDim='+reDim+'&showDimId='+showDimId+'&tenantId='+tenantId+'&config=' + config+"&groupIdList="+groupIdList,
		height:480,
		width:930,
		title:title,
		ondestroy:function(action){
...
function _UploadDialogShowFile(config){
	...
	url=encodeURI(url);
 	...
 	_OpenWindow({
        ...
        onload: function() {
        	var iframe = this.getIFrameEl();
            iframe.contentWindow.setFiles(files);
        },
        ondestroy: function(action) {
            if (action != 'ok') return;
            var iframe = this.getIFrameEl();
...
function _openDoc(fileId){
	var conf={officeId:fileId,doctype:"docx",version:false}
	openOffice(conf);
}  
...
function openBpmOpinionsDialog(instId){
	var url=__rootPath+"/bpm/core/bpmInst/opinions.do?instId="+ instId;
	_OpenWindow({
		title:'审批历史',
		height:580,
		width:900,
...
function openInformDialog(conf){
	var url=__rootPath+'/sys/inform/sysInform/dialog.do';
	if(conf.single==undefined) conf.single=true;
	url+="?single=" + conf.single;
	if(conf.boDefId){
		url+="&boDefId=" + conf.boDefId;
...
function openInformPdfDialog(conf){
	var url=__rootPath+'/sys/inform/sysInformPdf/dialog.do';
	_OpenWindow({
		title:'选择通知表单模板',
		height:600,
		width:800,
 ...
 * @returns
 */
function openCategoryDialog(catKey,callback){
    _OpenWindow({
        url:__rootPath+'/sys/core/sysTree/dialog.do?catKey='+catKey,
        title:'选择分类',
        height:600,
        width:400,
        ondestroy:function(action){
 ...
 * @param conf
 */
function openFormula(conf){
    _OpenWindow({
        title:'公式设计器',
        iconCls:'icon-formula',
        url:__rootPath+'/bpm/core/bpmSolution/expEditor.do?solId='+conf.solId+'&nodeId='+conf.nodeId+'&actDefId='+conf.actDefId,
        width:800,
        height:700,
        onload:function(){
            var win=this.getIFrameEl().contentWindow;
            win.addExp(conf.setting);
        },
        ondestroy:function(action){
            if(action!='ok') return;
            var win=this.getIFrameEl().contentWindow;
